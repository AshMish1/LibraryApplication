...
//display the menu options to hte user
	static void displayMenu(){
		if(LibraryOperation.bookList.isEmpty()){
			displayFirstMenu();
			LibraryOperation.menuChoice = LibraryOperation.readInteger(Messages.choiceMessage, Messages.tryAgainMessage, 0, 5);
		}else if(LibraryOperation.bookList.size() > 0){
			displaySecondMenu();
			LibraryOperation.menuChoice = LibraryOperation.readInteger(Messages.choiceMessage, Messages.tryAgainMessage, 0, 6);
		}
	}
	 ...
	 */
	static void addBook(){
		LibraryItem newBook = new LibraryItem(title, author, genre); //create new book object with status "Available."
		newBook.donateBook();
	...
	public static void borrowBook(){
		...
		bookChoice = (LibraryOperation.readInteger(Messages.enterBookIndexMessage, Messages.bookIndexNotInListMessage, 0, LibraryOperation.bookList.size()));
		
		borrowLoop:
		while(LibraryOperation.menuChoice == 3){
	...
	static void returnBook(){
		...
		if(titleExistance == false){
			boolean repeatReturnBook = LibraryOperation.readYesNo("<ERROR> The book with the title "+"\""+returnBookTitle +"\""+ "wasn't found!"
	...
	public static void removeBook(){
		...
		bookChoice = LibraryOperation.readInteger(Messages.enterRemoveBookIndex ,Messages.bookIndexNotInListMessage + Messages.tryAgainMessage, 0, bookList.size());		
		int p = 0;
		while (p < borrowedBooks.size()){//Search for the book by title, if it exists change it's status,
			//it's borrower and borrowDate.
	...
	//chech if the user wants yes or no
	...
	public void run() {
		...
		LibraryOperation.displayMenu();// Displays the main menu and ask for choice.
		...
		exit: while (LibraryOperation.menuChoice != 0) {
			...
			if (LibraryOperation.menuChoice == 3 && LibraryOperation.bookList.size() > 0 && LibraryOperation.borrowedBooks.size() > 0) {
